...
function check_min(field,min_len)
{
	if(document.getElementById(field).value.length < min_len)
		return false;
	else
		return true;
}

function generate_hash(field)
{
	return generate_hash(field,0)
}

function generate_hash(field,min_len)
{
	if(min_len > 0)
	{
		if(check_min(field,min_len))
		{
			var pass = document.getElementById(field)
			pass.value = sha256Hash(pass.value)
		}
		else
		{
//			alert('Field '+field+' should not be less then '+min_len);
			return false;
		}
	}
	else
