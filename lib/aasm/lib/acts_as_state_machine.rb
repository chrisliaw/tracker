    ...
    module StateMachine              #:nodoc:
        ...
        class Event
          ...
          def is_from?(record)
            @transitions.each do |tr|
              if tr.from.to_sym == record.state.to_sym and tr.guard(record)
                return true
              end
            end
            false
          ...
          def go_to?(record)
            @transitions.each do |tr|
              if tr.to.to_sym == record.state.to_sym and tr.guard(record)
                return true
              end
            end
            false
      ...
      module InstanceMethods
        ...
        def run_initial_state_actions
          ...
          raise InvalidState if initial == nil
        ...
        ## added by Chris
        # Returns the next possible events to allow UI shows all possible next actions
        def get_next_possible_event(record)
          @possible_events = []
          et = self.class.event_table
          #et = self.class.read_inheritable_attribute(:event_table)
          et.keys.each do |e|
            evt = et[e.to_sym]
            if evt.is_from?(record)
              @possible_events << evt.name
            end
          end
          @possible_events
        end
